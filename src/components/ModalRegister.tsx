...
export default function ModalRegister() {
  ...
  const inputAgreeOnChange = (event: React.ChangeEvent<HTMLInputElement>) => {
  ...
  const computeTotalPayment = () => {
    ...
    if (buyBottle) totalItem += 200;
    if (buyShoes) totalItem += 600;
    if (buyCap) totalItem += 400;

    if (buyBottle && buyShoes && buyCap) {
      totalItem *= 0.8; // Apply 20% discount
    }
